...
pub struct SendFuture<T: AsRef<[u8]>> {
...
impl<T: AsRef<[u8]> + 'static> Future for SendFuture<T> {
    type Output = i32;
            ...
            match shared_state.cqes.get() {
                ...
                None => {
                    shared_state.waker.register(cx.waker());
                    {
                        ...
                        let send_e =
                            opcode::Send::new(types::Fd(self.fd), buf.as_ptr(), buf.len() as u32)
                                .build()
                                .user_data(id);
